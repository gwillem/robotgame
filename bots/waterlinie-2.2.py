...
class Robot():
    ...
    def is_spawn_imminent(self, within=0):
        turns_to_spawn = 10 - (self.turn % 10)
        return  turns_to_spawn <= within
...
#~ 
                ...
                #~ if is_spawn(dst):
                    ...
                    #~ if not is_spawn(src):
                        #~ base_move_score -= 5
                    #~ if self.is_spawn_imminent(t=0) and not is_spawn(src):
                        #~ base_move_score -= 1000
                    #~ if is_spawn(src):
                        #~ base_move_score += 5
                ...
                proposals.add_move(base_move_score, 'move', src, dst)
